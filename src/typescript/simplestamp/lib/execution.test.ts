import * as fs from 'fs';
import * as path from 'path';

import { Attestation } from '../models/simplestamp/v1/attestation';
import { OperationType } from '../models/simplestamp/v1/operation';
import { AttestationStatus } from '../models/simplestamp/v1/status';
import { Execution } from './execution';
import { Parser } from './parser';

const DATA = path.join(__dirname, '../../../tests/data');

const hash = Buffer.from(
  '093febd0f49a812931239b920acabbab20c3511a49d44e79effba8d44ec2b102',
  'hex',
);

const binDigest = fs.readFileSync(path.join(DATA, 'digest-01-request.bin'));
const binTimestamp = fs.readFileSync(path.join(DATA, 'digest-01-response.bin'));
const operations1 = Parser.parseServerResponse(binDigest);
const operations2 = Parser.parseServerResponse(binTimestamp);

const binDigestBob = fs.readFileSync(path.join(DATA, 'digest-02-request.bin'));
const operationsBob = Parser.parseServerResponse(binDigestBob);

describe('Execution: Executing operations on a valid digest request', () => {
  test('.processOperations of valid data extracts correct values', () => {
    const attestation = Attestation.create({ operations: operations1 });
    const processed = Execution.processOperations(hash, attestation);

    expect(processed.calendarUrl).toBe('https://bob.btc.calendar.opentimestamps.org');
    expect(processed.status).toBe(AttestationStatus.ATTESTATION_STATUS_PENDING);
    expect(processed.blockHeight).toBe(0);
    expect(Buffer.from(processed.blockMerkleRoot).length).toBe(0);
    expect(Buffer.from(processed.transactionId).length).toBe(0);
    expect(Buffer.from(processed.timestampMerkleRoot).length).toBe(0);

    expect(Buffer.from(Attestation.encode(processed).finish()).toString('hex'))
      .toBe('');
  });

  test('.deriveCalendarKey of valid data produces correct value', () => {
    expect(Execution.deriveCalendarKey(hash, operations1).toString('hex'))
      .toBe('5e6d10ee8c8db23b3cf21796e1fcb74499d8d756653ab6134d4fcf92c6f0bb61f0a37f3d21de054589a79bd4');
  });

  test('.deriveCalendarKey of more valid data produces correct value', () => {
    expect(Execution.deriveCalendarKey(hash, operationsBob).toString('hex'))
      .toBe('5e67f3c2426f90c5f58d718639f1b5f90c7890ec42e6ee6ae224ad479a7a4af9a80090c36c72333bc750ad77');
  });

  test('.deriveCalendarKey without an attestation operation', () => {
    const operations = operations1.slice(0, operations1.length - 2);
    expect(Execution.deriveCalendarKey(hash, operations).toString('hex'))
      .toBe('5e6d10ee8c8db23b3cf21796e1fcb74499d8d756653ab6134d4fcf92c6f0bb61f0a37f3d');
  });
});

describe('Execution: Executing operations on a valid timestamp request', () => {
  test('.processOperations of valid data extracts correct values', () => {
    const attestation = Attestation.create({ operations: operations1.concat(operations2) });
    const processed = Execution.processOperations(hash, attestation);

    expect(processed.calendarUrl).toBe('https://bob.btc.calendar.opentimestamps.org');
    expect(processed.status).toBe(AttestationStatus.ATTESTATION_STATUS_BITCOIN);
    expect(processed.blockHeight).toBe(621080);
    expect(Buffer.from(processed.blockMerkleRoot).toString('hex'))
      .toBe('b2b93b85a0af4e8b58971ac9447c6906a2ed0105f8af60a58c74dacc9076a9d5');
    expect(Buffer.from(processed.transactionId).toString('hex'))
      .toBe('226f1fd3b351c249324530bdf71df7954444c6bbd636c51ea2b1234b1447083e');
    expect(Buffer.from(processed.timestampMerkleRoot).toString('hex'))
      .toBe('b2e8f8289604511a089390302bc2ffc409e959a833bd043974d3688d3ff785b9');

    expect(Buffer.from(Attestation.encode(processed).finish()).toString('hex'))
      .toBe('');
  });

  test('.processOperations of sha1 and ripemd160', () => {
    expect(() => {
      const hexRequest = binDigest.toString('hex').replace('08', '02');
      const operations = Parser.parseServerResponse(Buffer.from(hexRequest, 'hex'));
      Execution.deriveCalendarKey(hash, operations);
    }).not.toThrow();

    expect(() => {
      const hexRequest = binDigest.toString('hex').replace('08', '03');
      const operations = Parser.parseServerResponse(Buffer.from(hexRequest, 'hex'));
      Execution.deriveCalendarKey(hash, operations);
    }).not.toThrow();

    expect(() => {
      const hexRequest = binDigest.toString('hex').replace('08', '02');
      const operations = Parser.parseServerResponse(Buffer.from(hexRequest, 'hex'));
      const attestation = Attestation.create({ operations });
      Execution.processOperations(hash, attestation);
    }).not.toThrow();

    expect(() => {
      const hexRequest = binDigest.toString('hex').replace('08', '03');
      const operations = Parser.parseServerResponse(Buffer.from(hexRequest, 'hex'));
      const attestation = Attestation.create({ operations });
      Execution.processOperations(hash, attestation);
    }).not.toThrow();
  });

  test('.processOperations with an invalid operation type throws', () => {
    expect(() => {
      const operations = Parser.parseServerResponse(binDigest);
      // Inject an invalid type into the first operation
      operations[0] = { ...operations[0], type: 123 as OperationType };
      const attestation = Attestation.create({ operations });
      Execution.processOperations(hash, attestation);
    }).toThrow();
  });
});
