...
describe('Parser: Parsing the response to a digest server request', () => {
  test('.parseServerResponse of valid alice digest binary data', () => {
    ...
    expect(Buffer.from(operations[2].value).toString('hex')).toBe('5e6d10ee');
    expect(Buffer.from(operations[3].value).toString('hex')).toBe('21de054589a79bd4');
    expect(operations[4].calendarUrl).toBe('https://bob.btc.calendar.opentimestamps.org');
  });

  test('.parseServerResponse of valid bob digest binary data', () => {
    const binDigest = fs.readFileSync(path.join(DATA, 'digest-02-request.bin'));
    const operations = Parser.parseServerResponse(binDigest);

    expect(operations.length).toBe(5);
    expect(Buffer.from(operations[2].value).toString('hex')).toBe('5e67f3c2');
    expect(operations[4].calendarUrl).toBe('https://bob.btc.calendar.opentimestamps.org');
  });
  ...
  test('.parseServerResponse with an invalid operation type to be set unknown', () => {
    ...
    expect(operations[4].status).toBe(AttestationStatus.ATTESTATION_STATUS_UNKNOWN);
  });
  ...
  test('.reverse a buffer properly for txids', () => {
    ...
    expect(Parser.reverse(hash).toString('hex')).toBe(
      '02b1c24ed4a8fbef794ed4491a51c320abbbca0a929b233129819af4d0eb3f09',
    );
  });
});
...
describe('Parser: Parsing the response to a timestamp server request', () => {
  test('.parseServerResponse of valid alice timestamp binary data', () => {
    ...
    expect(Buffer.from(operations[25].value).toString('hex')).toBe(
      '010000000145238927573033099b6c10c16a37657fdd891f2057e6a9e5c248c774c9b76c5d0000000000fdffffff02fe5b0500000000001600142a8d531f9f574785bd735fb3b8bddd5789a78e090000000000000000226a20',
    );
    expect(Buffer.from(operations[26].value).toString('hex')).toBe('177a0900');
  });

  test('.parseServerResponse of valid bob timestamp binary data', () => {
    const binTimestamp = fs.readFileSync(path.join(DATA, 'digest-02-response.bin'));
    const operations = Parser.parseServerResponse(binTimestamp);

    expect(operations.length).toBe(66);
    expect(operations[65].blockHeight).toBe(621138);
  });

  test('.parseServerResponse with an invalid operation type', () => {
    const binDigest = fs.readFileSync(path.join(DATA, 'digest-01-response.bin'));
