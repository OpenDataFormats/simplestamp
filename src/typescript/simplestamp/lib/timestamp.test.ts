...
const calendarKey = Buffer.from(
  '5e7505e786c61440e52ff8a9d903531ed45b433b055754e656b712b3aba7bae20e1aca765e87ee6d676f0a9d',
  'hex',
);
...
describe('Timestamp: Static methods produce deterministic results', () => {
  ...
  test('constructor with the same hash produces different digests from random nonces', () => {
    ...
    expect(t1.getDigestHash().equals(t2.getDigestHash())).toBe(false);
  });
});

describe('Timestamp: Binary serialization and deserialization work', () => {
  test('.fromBinary produces a valid Timestamp', () => {
    expect(() => { Timestamp.fromBinary(binTimestamp1); }).not.toThrow();

    const t = Timestamp.fromBinary(binTimestamp1);

    expect(Buffer.from(t.timestamp_.hash).toString('hex'))
      .toBe('54df0ea7227fdd5f1b3b7d88448fe23596de4270196d5d55fc32c595ad5e8bdd');
    expect(Buffer.from(t.timestamp_.nonce).toString('hex')).toBe(nonce);
    expect(t.getDigestHash().toString('hex'))
    ...
    expect(importedAgain).toBe(false);

    expect(() => { t.getAttestationsByKey(calendarKey); }).not.toThrow();
    expect(() => {
      t.getAttestationsByKey(Buffer.from('I am not a valid calendar key'));
    }).toThrow();

    const json = t.toJSON();
    expect((json as Record<string, string>).hash)
      .toBe('54df0ea7227fdd5f1b3b7d88448fe23596de4270196d5d55fc32c595ad5e8bdd');

    expect(() => { Timestamp.fromBinary(Buffer.alloc(128, 9)); }).toThrow();
  });
...
describe('Timestamp: Setting rich details are included in the digest hash', () => {
  ...
  test('Adding identity information produces a valid digest', () => {
    ...
    t.setIdentity(
      'US', 'NY', 'New York', 'Rays Pizza', 'Engineering',
      undefined as unknown as string,
      'rayspizza@example.com', 'Famous Rays',
    );
    expect(t.getDigestHash().toString('hex'))
      .toBe('4bc307193a5180f740e73cf0cd593bd8a3b0f61b20419ef180664556c8fd1c37');

    const json = t.toJSON() as { identity: Record<string, string> };
...
describe('Timestamp: Location data is set correctly', () => {
  test('Wrapper method sets underlying values', () => {
    ...
    const l = t.timestamp_.location!;

    expect(typeof l.latitude).toBe('number');
    expect(typeof l.longitude).toBe('number');
    expect(typeof l.altitude).toBe('number');
    expect(typeof l.accuracyMeters).toBe('number');
    expect(typeof l.direction).toBe('number');
    expect(typeof l.velocity).toBe('number');
  });

  test('Setting location after stamping throws an error', () => {
    expect(() => {
      const t = Timestamp.fromBinary(binTimestamp1);
      t.setLocation(3.14159);
    }).toThrow();
  });

  test('Setting location information produces a valid digest', () => {
    const t = Timestamp.fromBinary(binTimestamp0);
    t.setLocation(3.14159, 3.14159);
    expect(t.getDigestHash().toString('hex'))
      .toBe('32635c36c77540d5c0a233e821af91f9322014bf6ce1497921edb18982e81f09');

    const json = t.toJSON() as { location: Record<string, number> };
